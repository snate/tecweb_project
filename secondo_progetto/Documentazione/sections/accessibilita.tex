...
adiacente a un altro senza altri elementi o tag tra i due link, perciò 9.5 è
rispettato.
\end{enumerate}
...
prevista dall'ente, presumibilmente è anche disapprovata (\textit{Priorità 2}); %CONTROLLARE
...
le pagine non rispettavano gli standard, perciò non sono fornite versioni
alternative \textit{accessibili} delle pagine del sito (\textit{Priorità 1}).
\end{enumerate}
...
\item Sono presenti pochi \textbf{meta tag}, ritenuti insufficienti poichè
mancano informazioni importanti tra cui autore e tipo di contenuto
(\textit{Priorità 2});
...
\footnote{\texttt{https://trendimpulse.wordpress.com/tag/multiple-lines-ascii-art/}}.
\end{enumerate}

\subsubsection{Assicurarsi che i documenti siano chiari e semplici}
\begin{enumerate}
\item Anche se il testo non sempre è grammaticalmente corretto e non sia
semplice, si ritiene che la (\textbf{seconda}) scelta sia dovuta al fatto che
l'audience è un pubblico di cultura letteraria medio-alta (essendo il sito di
una biblioteca), perciò 14.1 si può ritenere rispettato;
...
dal momento che se perdono il focus dopo averlo acquisito (e aver perso il
contenuto del segnaposto) e sono vuoti, non ricompare il placeholder.
\end{itemize}

\subsubsection{Separazione tra elementi del sito}
Qui di seguito vengono evidenziate le incongruenze rilevate nel sito rispetto
alla separazione tra i tre elementi che lo compongono, ovvero
\textbf{struttura}, \textbf{presentazione} e \textbf{comportamento}.

\paragraph{Separazione struttura-comportamento} ~\\
BLA BLA BLABLABLA BLA BLA

\paragraph{Separazione struttura-presentazione} ~\\
Alcune classi, come \textit{shiftsx} in \texttt{contatti.html} forniscono informazioni di carattere
presentazionale e non semantico.
