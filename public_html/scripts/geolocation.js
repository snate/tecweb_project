...
// creati dinamicamente.
    ...
    _.each(filteredloc,function(località){
      return geodata.appendChild(document.createElement('p'));
    });
...
// Creo un array con le località riordinate per distanza minore dalla posizione attuale.
    ...
    if(flag){
      ...
      _.each(sortedlist,function(sortedlist){
        return  geodata.childNodes[k++].innerHTML = sortedlist.name + " : " + sortedlist.distance + " Km";
      });
    }else
      geodata.childNodes[0].innerHTML = currentloc.name + " è lontana " + currentloc.distance + " Km da te.";
  };
